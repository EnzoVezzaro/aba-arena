 ...
 * — with live streaming, provider/model choice per panel, and the ability
 * to swap the repository at any time. Add --headless to run a single
 * benchmark from the terminal instead.
 *
 * Benchmarks always run against a snapshot copy, so the original repository
 * is never modified. Docker is optional: when it is available the benchmark
 * runs in an isolated container; otherwise (or with --local) it runs
...
async function runBattle(argv) {
  ...
  const { LocalSandbox, createSandbox } = require('./sandbox.cjs');
  let sandbox = await createSandbox(config.sandbox, { local: config.sandbox.local });
  if (sandbox.backend === 'docker') {
    console.log(`  Sandbox mode: Docker (${sandbox.config.image})`);
  } else {
    console.log(`  Sandbox mode: local host (no Docker, Node ${process.version})`);
  }
  console.log(`  Network policy: ${sandbox.config.network}`);
  console.log(`  Original repository protected: ✓`);

  // Local sources get a snapshot copy; GitHub/git clones live under the
  // ABA sandbox dir and are already isolated copies.
  const workDir = importResult.snapshotDir || importResult.originalDir;

  let containerInfo;
  try {
    containerInfo = await sandbox.start(workDir, importResult.snapshotInfo.commitSha);
    console.log(`  ${sandbox.backend === 'docker' ? 'Container' : 'Sandbox'} started: ${containerInfo.containerId.slice(0, 12)}`);
  } catch (err) {
    if (sandbox.backend === 'docker') {
      console.warn(`  Docker sandbox failed (${err.message})`);
      console.warn('  Falling back to the local host sandbox (no container isolation).');
      sandbox = new LocalSandbox(config.sandbox);
      containerInfo = await sandbox.start(workDir, importResult.snapshotInfo.commitSha);
    } else {
...
function main() {
  ...
  // Default: spawn the battle arena web app (Vite UI) and open the browser.
  // --headless keeps the terminal benchmark flow.
  if (!args.headless) {
    const { startServer } = require('./server.cjs');
    startServer({ open: true, initialSource: args.project }).catch((err) => {
      console.error(`Failed to start ABA: ${err.message}`);
      process.exit(1);
    });
    return;
  }
